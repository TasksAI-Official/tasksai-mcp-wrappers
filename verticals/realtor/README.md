...
```bash
npm exec --package=@tasksai/install --call 'tasksai-install realtor --source https://github.com/TasksAI-Official/tasksai-mcp-wrappers/tree/main/verticals/realtor --client claude-desktop'
```
...
```bash
npm exec --package=@tasksai/install --call 'tasksai-install realtor doctor --client claude-desktop'
```
