# HouseholdTasksAI Troubleshooting
...
The manifest must identify product `household`, domain `householdtasksai.com`, and
...
Confirm the browser is on `https://householdtasksai.com/connect` and that the
...
`TASKSAI_INSTALL_DIR` may point to an exact alternate HouseholdTasksAI install
...
ask: `Check my HouseholdTasksAI credit balance.`

## A finished file is not visible

Ask the assistant to use `householdtasksai_save_document`. By default, files are
saved under `~/Documents/TasksAI/HouseholdTasksAI/`. If `TASKSAI_OUTPUT_DIR` is
set, check that configured folder. Local output does not appear on the
HouseholdTasksAI website.

## No credits are available

Visit https://householdtasksai.com/#pricing. If the balance appears incorrect,
contact hello@householdtasksai.com without sending farm notes, records, or
generated documents.
